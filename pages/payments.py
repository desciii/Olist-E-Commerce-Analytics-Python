...
def page_payments():
    return html.Div([
        ...
        html.P(
            "How are customers paying, and how much?",
...
def _update(types, max_install, min_val):
    ...
    avg_inst = (
        filt[filt["payment_installments"] > 0]["payment_installments"].mean()
        if not empty else 0
    )
    ...
    fig_install.update_layout(**PLOTLY_LAYOUT, title="Installment Distribution")

    # Tags
    tags = []

    for t in (types or []):
        tags.append(_tag(t.replace("_", " ").title(), "#fff0f0", C["accent3"], C["accent3"]))
